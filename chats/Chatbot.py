...
class Chatbot:
    ...
    def preload_conversation(self, conversation_history = None):
        if isinstance(conversation_history, list):
            ...
            self._conversation_history = [
                    self._construct_message('From now on, return the output as a JSON object that can be loaded in Python with the key as \'text\'. For example, {"text": "<output goes here>"}. Your name is AthenaAI from now on, and you have been developed by Emin Joy.'), # default of Google is set to Markdown therefore we change t to JSON
